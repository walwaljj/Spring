...
//    MemoryMemberRepository memberRepository = new MemoryMemberRepository();
    ...
    MemberService memberService; // 테스트를 하기위해 멤버 선언. 멤버는 service.
    MemoryMemberRepository memberRepository; // 메모리를 클리어시키기 위함.

    @BeforeEach
    public void beforeEach(){ // 테스트를 실행하기 전에

                            /** DI
                            //1.  memorymemberrepository()를 생성함고 멤버변수로 넘김 -> memberservice에 넘겨쥼

                            //   같은 memorymemberrepository 사용 가능 ==> 같은 DB를 사용.
                            //뭐가 좋은가 ? memorymemberrepository 입장에서 new 하지 않아도 되고 값을 넣어줌..*/
        //
    ...
    @AfterEach
    public void afterEach(){ // 실행 될때 마다 메모리를 클리어시켜줌.
    ...
    void 회원가입() { // join의 테스트
        ...
        member.setName("hello"); // 만약 spring 으로 변경하면 에러남.
